...
5. Fixed a bug that created non-standard zip files that could not be extracted by macOS archive utility.
6. Improved unlazy logic to pick a better image for unlazy.
7. Other bug fixes and improvements.

1.48 [2026-02-01]:
Major Update
1. Added new hotkeys for copying the current image:
  - Copy image: Ctrl + C
  - Copy URL  : Ctrl + Shift + C
2. Added new option to disable image backup.
  // useful when a website loads images in unusual ways and you want a fresh list
3. Added settings import / export functionality.
4. Improved wording and i18n consistency on the options page.
...
2. Fixed a bug that caused image transitions to flash in some conditions.
3. Added caching for canvas functionality, this should reduce potential bugs related to canvas.
4. Other bug fixes and improvements.

1.43 [2025-10-06]:
Performance and Stability update
1. Improved image collection logic, reduce initial delay.
2. Improved caching to reduce lag when the number of images increases.
3. Improved logic for restoring image index and transformation when restarting the viewer.
4. Rewrote image transformation code to improve performance and fix a rare bug that caused strange animations.
5. Fixed a bug that sometimes skipped the largest version of an image during the unlazy.
